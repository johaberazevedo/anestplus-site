...
return;
  ...
  return (
    <div className="grid gap-6">
      ...
      {/* Área de Upload - Visível apenas quando está aguardando documento */}
      {state.status === "idle" && (
        <div className="relative overflow-hidden rounded-3xl border-2 border-dashed border-slate-300 bg-slate-50 transition-colors hover:border-blue-400 hover:bg-blue-50/50">
          ...
          <div className="pointer-events-none flex flex-col items-center justify-center p-10 text-center">
            ...
            <p className="text-base font-semibold text-slate-900">Clique ou arraste um documento PDF</p>
            <p className="mt-1 text-sm text-slate-500">Para iniciar a verificação de autenticidade e autoria</p>
          </div>
        </div>
      )}
      ...
      {state.status === "loading" && (
        <div className="rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-4 text-sm font-medium text-slate-700">Lendo PDF e analisando integridade criptográfica...</p>
        </div>
      )}
      ...
      {state.status === "error" && (
        <div className="rounded-3xl border border-red-200 bg-red-50 p-6 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold uppercase tracking-wide text-red-800">Falha na validação</p>
              {/* whitespace-pre-wrap garante que as quebras de linha do seu log de erro funcionem */}
              <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-red-700">{state.message}</p>
            </div>
          </div>
          <div className="mt-6">
            <button
      ...
      {state.status === "success" && summary && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {/* Header de Autenticidade Confirmada */}
          <div className="flex flex-col items-start justify-between gap-4 rounded-3xl border border-emerald-200 bg-emerald-50 p-6 sm:flex-row sm:items-center">
            <div>
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-bold uppercase tracking-wide text-emerald-800">Autenticidade Confirmada</p>
              </div>
              <p className="mt-2 text-sm text-emerald-800">
                O documento <span className="font-semibold">{state.fileName}</span> é íntegro e sua assinatura digital é válida.
              </p>
            </div>
            <button
              onClick={handleReset}
          ...
          {/* Dados Principais da Assinatura (Foco na Informação Útil) */}
          <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center gap-2 border-b border-slate-200 bg-slate-900 px-6 py-4">
              <svg className="h-4 w-4 text-cyan-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.957 11.957 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
            ...
            <div className="grid gap-px bg-slate-100 sm:grid-cols-2">
              <InfoCell label="Profissional Responsável" value={summary.signer?.name ?? "—"} />
              <InfoCell label="Conselho (CRM)" value={summary.signer?.crm ?? "—"} />
              <InfoCell label="Especialidade (RQE)" value={summary.signer?.rqe ?? "—"} />
              <InfoCell label="Data e Hora da Assinatura" value={summary.signedAtISO8601 ? new Date(summary.signedAtISO8601).toLocaleString("pt-BR") : "—"} />
            </div>
          </div>

          {/* Acordeão de Dados Técnicos (Esconde a complexidade do JSON/Hash) */}
          <details className="group rounded-3xl border border-slate-200 bg-white shadow-sm [&_summary::-webkit-details-marker]:hidden">
            ...
            <div className="border-t border-slate-200 p-6 space-y-8">
              ...
              {/* Trust Layer Info */}
              <div>
                <h3 className="mb-4 text-xs font-bold uppercase tracking-widest text-slate-400">Trust Layer Criptográfica</h3>
                <div className="grid gap-3 sm:grid-cols-2">
                  <InfoRow label="Algoritmo" value={summary.crypto?.algorithm ?? "—"} mono />
                  <InfoRow label="Versão do Schema" value={String(summary.schemaVersion ?? "—")} mono />
                  <InfoRow label="Payload Hash (SHA-256)" value={summary.crypto?.payloadHashSHA256 ?? "—"} mono />
                  <InfoRow label="Chave Pública (Base64)" value={summary.crypto?.publicKeyBase64 ?? "—"} mono />
                  <div className="sm:col-span-2">
                    <InfoRow label="Assinatura Digital (Signature)" value={summary.crypto?.signatureBase64 ?? "—"} mono />
                  </div>
                </div>
              </div>

              {/* Acordeões Secundários para Logs Extensos */}
...
// Componente para a Grid principal do Sucesso
...
// Componente original atualizado visualmente para a área técnica
function InfoRow({
  label,
  value,
  mono = false,
...
// Funções originais mantidas 100% intactas
